...
export class FilterCommercePipe implements PipeTransform {
  transform(value: any, arg: any): any {
    if (arg == '' || arg.length < 1)
      return (document.getElementById('tabla-buscador').style.display = 'none');
    const result = [];
    for (const negocio of value) {
      ...
      if (
        ('peluqueria'.indexOf(arg.toLowerCase()) > -1 &&
          negocio.businessType == 'HAIRDRESSER') ||
        (('restaurante'.indexOf(arg.toLowerCase()) > -1 ||
          'bar'.indexOf(arg.toLowerCase()) > -1) &&
          negocio.businessType == 'RESTAURANT') ||
        ('otros'.indexOf(arg.toLowerCase()) > -1 &&
          negocio.businessType == 'GENERAL')
      ) {
